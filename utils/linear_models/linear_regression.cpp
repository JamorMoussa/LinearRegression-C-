#include <iostream>
#include "linear_regression.h"
#include "../data/data_loader.h"



LinearRegression::LinearRegression(double lr, int n_iters){
    this->lr = lr;
    this->n_iters = n_iters;
}

LinearRegression::~LinearRegression(){

}

void LinearRegression::fit(DataFrame X_train, DataFrame y_train){
    this->X_train = X_train;
    this->y_train = y_train;

    auto [n_samples, n_features] = X_train.shape();

    weights = Eigen::MatrixXd::Random(n_features, 1);
    bias = Eigen::MatrixXd::Zero(n_features, 1);

    // std::cout << weights << std::endl;

    for(int i = 0; i< n_iters; i++){
        auto y_pred = X_train.data * weights + bias;

        auto dw = (1 / n_samples) * X_train.data.transpose() * (y_pred - y_train.data);
        auto db = (1 / n_samples) * (y_pred - y_train.data);

        weights -= lr * dw;
        bias -= lr * db;
    }

}